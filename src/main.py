...
def main(config_path:Config, args:ArgumentParser):
    ...
    if (args.cont and args.mode == 'train') or args.mode != 'train':
        ...
        os.makedirs(base_path+'data/wiki-split/processed', exist_ok=True)
    
        # save nmt data
        save_data(base_path)

        # check tokenizer file
        if not os.path.isdir(base_path+'data/wiki-split/tokenizer/vocab_'+str(config.vocab_size)):
            print('You must make vocab and tokenizer first..')
            sys.exit()
        config.tokenizer_path = base_path+'data/wiki-split/tokenizer/vocab_'+str(config.vocab_size)+'/vocab.txt'
